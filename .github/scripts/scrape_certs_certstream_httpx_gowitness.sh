...
#---------------------# 
#HTTPx (General)
 httpx -l "$GITHUB_WORKSPACE/main/Data/bd_ccTLDs/certstream_domains_bd_all_24h.txt" -r "/tmp/resolvers.txt" -ip -status-code -title -location -follow-host-redirects -include-chain -server -tech-detect -cdn -content-type -content-length -websocket -random-agent -filter-regex "The plain HTTP request was sent to" -disable-update-check -silent -no-color | tee "$GITHUB_WORKSPACE/main/Data/bd_ccTLDs/certstream_domains_bd_all_24h_httpx.txt"
#Git Pull
 cd "$GITHUB_WORKSPACE/main" && git pull origin main  
#---------------------# 
#GoWitness
